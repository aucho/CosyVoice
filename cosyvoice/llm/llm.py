...
class TransformerLM(torch.nn.Module):
    ...
    def sampling_ids(
        ...
        while True:
            ...
            if top_ids < 0 or top_ids >= weighted_scores.size(0):
                print(
                    f"Warning: top_ids {top_ids} out of range [0, {weighted_scores.size(0)-1}]"
                )
            ...
            if num_trials > max_trials:
                # 添加更详细的调试信息
                print(
                    f"Warning: sampling reached max_trials {max_trials}, weighted_scores shape: {weighted_scores.shape}"
                )
                print(
                    f"speech_token_size: {self.speech_token_size}, top_ids: {top_ids}"
                )
                # 强制选择一个非EOS的token
                non_eos_indices = torch.where(
                    weighted_scores != self.speech_token_size
                )[0]
    ...
    def inference(
        ...
        actual_text_len = text_len - prompt_text_len
        # 确保 actual_text_len 不为负数或0
        if actual_text_len <= 0:
            actual_text_len = max(1, text_len.item())  # 至少使用 text_len，确保不为0
            logging.warning(
                f"text_len ({text_len.item()}) <= prompt_text_len ({prompt_text_len.item()}), using text_len as fallback"
            )

        # 计算基础长度
        base_min_len = int(actual_text_len * min_token_text_ratio)
        base_max_len = int(actual_text_len * max_token_text_ratio)

        # 动态调整冗余：根据文本长度智能调整冗余比例，平衡生成完整性和性能
        actual_text_len_value = actual_text_len.item() if isinstance(actual_text_len, torch.Tensor) else actual_text_len
        if actual_text_len_value < 50:
            # 短文本：减少冗余，避免过度生成，提高效率
            min_len_redundancy = max(30, int(base_min_len * 0.5))  # 至少30个token，或50%冗余
            min_len = max(50, base_min_len + min_len_redundancy)  # 至少保证50个token
            max_len_redundancy = max(50, int(base_max_len * 0.5))  # 至少50个token，或50%冗余
        elif actual_text_len_value < 200:
            # 中等文本：正常冗余，平衡质量和性能
            min_len_redundancy = max(80, int(base_min_len * 1.0))  # 至少80个token，或100%冗余
            min_len = max(100, base_min_len + min_len_redundancy)  # 至少保证100个token
            max_len_redundancy = max(100, int(base_max_len * 1.0))  # 至少100个token，或100%冗余
        else:
            # 长文本：增加冗余，确保完整生成，防止提前结束
            min_len_redundancy = max(150, int(base_min_len * 1.5))  # 至少150个token，或150%冗余
            min_len = max(150, base_min_len + min_len_redundancy)  # 至少保证150个token
            max_len_redundancy = max(150, int(base_max_len * 1.5))  # 至少150个token，或150%冗余
        ...
        try:
            ...
            # 记录实际生成长度，用于优化冗余参数
            actual_generated_len = len(out_tokens)
            if actual_generated_len < min_len:
                logging.warning(
                    f"Generated {actual_generated_len} tokens, less than min_len={min_len}. "
                    f"This may indicate premature termination."
                )
            elif actual_generated_len > min_len * 1.5:
                logging.debug(
                    f"Generated {actual_generated_len} tokens, significantly more than min_len={min_len}. "
                    f"Consider reducing redundancy for text_len={actual_text_len_value:.1f}."
                )
            else:
                logging.debug(
                    f"Generated {actual_generated_len} tokens (min_len={min_len}, max_len={max_len}). "
